...
sys_exofork(void)
{
	...
	struct Env * new_env;
    int flag = env_alloc(&new_env, curenv->env_id);
    if (flag < 0) {
        return flag;
    }
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
    ...
    if (!(perm & PTE_U) || !(perm & PTE_P) || (perm & ~PTE_SYSCALL) ||
            (uint32_t)va >= UTOP || PGOFF(va))
        return -E_INVAL;
    struct PageInfo * new_page = page_alloc(ALLOC_ZERO);
    if (!new_page)
        return -E_NO_MEM;
    struct Env * new_env;
    if (envid2env(envid, &new_env, perm) < 0)
        return -E_BAD_ENV;

    if (page_insert(new_env->env_pgdir, new_page, va, perm) < 0) {
        page_free(new_page);
        return -E_NO_MEM;
    }
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if (envid2env(srcenvid, &srcenv, 1) < 0 ||
	    ...
	    return -E_BAD_ENV;
	pte_t * pgentry;
	struct PageInfo * page = page_lookup(srcenv->env_pgdir, srcva, &pgentry);
//	if (!(*pgentry & PTE_W)) {
//	    panic("panic here");
//	}
	if (!page || (!(*pgentry & PTE_W) && (perm & PTE_W)))
        return -E_INVAL;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if ((uint32_t)srcva < UTOP && (PGOFF(srcva) || !page ||
            (perm & PTE_W) != (*pte & PTE_W) ||
            !(perm & PTE_U) || !(perm & PTE_P) ||
            (perm & (~PTE_SYSCALL))))
	    return -E_INVAL;
	int flag;
	if ((uint32_t)srcva < UTOP && (flag = page_insert(env->env_pgdir, page, env->env_ipc_dstva, perm)) < 0) {
	    return flag;
	}
